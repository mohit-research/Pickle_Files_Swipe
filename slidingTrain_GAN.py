...
results = []
file_ptr = open("results_GAN_KNN_tab.out", "w")
for val in range(1, 117):
        ...
        #X_matrix, y_vector = BorderlineSMOTE(random_state=0).fit_resample(X, y)
        #X_matrix, y_vector = SVMSMOTE().fit_resample(X, select_user)
        #undersample = NeighbourhoodCleaningRule(n_neighbors=3, threshold_cleaning=0.5)
        #X_matrix, y_vector = undersample.fit_resample(X, y)
        ...
        y = unpickling("Pickle_Files_Swipe/features_Y.pkl")
        ...
        #X_test = unpickling("X"+str(val)+".pkl")
        #y_test = unpickling("Y"+str(val)+".pkl")
        ...
        pickling("scaler_GAN_Tab.pkl", scaler)
        pickling("transformer_GAN_Tab.pkl", transformer)
        ...
        models = ["KNN"]#["RForest", "NB", "XGBoost", "MLP", "SVM"]
        # models = ["SVM"]
